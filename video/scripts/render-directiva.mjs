#!/usr/bin/env node
/**
 * Renders the MUNCAS "7 palabras, 20 intentos" reel and masters its audio for social.
 *
 *   node scripts/render-directiva.mjs
 *   node scripts/render-directiva.mjs --quality        # bigger file, crf 16
 *
...
const raw = join(outDir, "MUNCAS_DIRECTIVA_FINAL.raw.mp4");
const final = join(outDir, "MUNCAS_DIRECTIVA_FINAL.mp4");
...
const crf = args.includes("--quality") ? "16" : "19";
...
const render = run(
  "npx",
  [
    "remotion", "render", "MuncasDirectiva", raw,
    `--crf=${crf}`,
